...
type_data* archetype_new(world_t* wld, type_t components) {
	...
	event_post((event_t) { .type = on_addtype, .p0.data = new_archetype->type });
...
void archetype_delete(world_t* wld, type_data* archetype) {
	...
	event_post((event_t) {.type = on_deltype, .p0.data = archetype->type });
