...
static void character_callback(GLFWwindow* window, unsigned int codepoint) {
	event_post(eventhandler_instance(), (event_t) { .type = on_text, .p0.data = codepoint });
}
...
static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
	if (key >= 0 && key < 1024) {
		if (action == GLFW_PRESS || action == GLFW_REPEAT) {
			...
			if (k) event_post(eventhandler_instance(), (event_t) { .type = on_text, .p0.data = k });
		}
		double time = get_time();
		if (action == GLFW_PRESS) {
			app_instance()->keys[key] = 1;
			event_post(eventhandler_instance(), (event_t) { .type = on_press, .p0.data = key, .p1.nr = time });
		}
		else if (action == GLFW_RELEASE) {
			app_instance()->keys[key] = 0;
			event_post(eventhandler_instance(), (event_t) { .type = on_release, .p0.data = key, .p1.nr = time });
		}
	}
}

static void close_callback(GLFWwindow* window) {
	event_post(eventhandler_instance(), (event_t) { .type = on_quit });
}
