...
void app_init(app_t* manager, const char* name) {
	...
	event_register(manager, quit, app_stop, NULL);
}
...
void event_poll_cb(uv_idle_t* handle) {
	...
	if (manager->is_running) {
		...
		while (get_time() > manager->next_game_tick && loops < MAX_FRAMESKIP) {
			...
			event_post((event_t) { .type = on_preupdate, .p0.nr = interpolation });
			event_post((event_t) { .type = on_update, .p0.nr = interpolation });
			event_post((event_t) { .type = on_postupdate, .p0.nr = interpolation });
		...
		event_post((event_t) { .type = on_predraw, .p0.nr = dt });
		event_post((event_t) { .type = on_draw, .p0.nr = dt });
		event_post((event_t) { .type = on_postdraw, .p0.nr = dt });

		event_post((event_t) { .type = on_pregui, .p0.nr = dt });
		event_post((event_t) { .type = on_gui, .p0.nr = dt });
		event_post((event_t) { .type = on_postgui, .p0.nr = dt });
