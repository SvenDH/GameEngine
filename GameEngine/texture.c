...
int w_texture_load(lua_State* L) {
	...
	texture_t* texture = (texture_t*)resource_get(rm, *(rid_t*)lua_touserdata(L, 1));
	...
	image = (image_t*)resource_get(rm, *(rid_t*)lua_touserdata(L, 2));
	w = luaL_optinteger(L, 3, image->width);
	h = luaL_optinteger(L, 4, image->height);
