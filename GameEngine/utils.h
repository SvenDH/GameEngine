...
/* Saves or reuses lua string for fast pointer comparison */
inline const char* intern_luastring(lua_State* L, const char* string, size_t len) {
	static int luastring_counter = 1; //Lua arrays start at 1
#define StringTable "_Strings"
	if (lua_getglobal(L, StringTable) != LUA_TTABLE) {
		lua_newtable(L);
		lua_pushvalue(L, -1);
		lua_setglobal(L, StringTable);
	}
	const char* ptr = lua_pushlstring(L, string, len);
	lua_rawseti(L, -2, luastring_counter++);
	lua_pop(L, 1);
	return ptr;
}
...
inline int32_t getrightbitindex(int32_t v) {
